...
\todo{Removal of existing clones: How does this is possibly redundant thing work?}
...
\begin{tabular}{|l|l|l|}
\hline
Node (n) & Equality Array (E)     & Size (s) \\ \hline
1        & {[}1, -2, 3, -5, -6{]} & 5        \\ \hline
2        & {[}1, -2, 7, -5, -6{]} & 5        \\ \hline
3        & {[}1, -2, 3, -5, -6{]} & 5        \\ \hline
4        & {[}8, -2, 7, -5, -6{]} & 5        \\ \hline
...
In the example of figure \ref{fig:equalityarrays} the fragment on the left (consisting of n_1 and n_2) and the fragment on the right (consisting of n_3 and n_4) are two clone instances of a clone class. Note that the size of the equality arrays of all clone instances must equal the size of the equality arrays of the other clone instances for them to be considered a clone class. We calculate the variability of this clone instance as follows:



\begin{equation}\label{eq:variabilityclonerefactor}
\frac{|\{x>0 \and y>0 \and x \neq y | x \in E_1 \union E_2, y \in E_3 \union E_4\}|}{(s_1+s_2)+(s_3+s_4)}
\end{equation}

The symbols in this equation refer to the symbols of the column headers of table \ref{table:equalityarrays}.
...
E_1\text{ is cloned with }E_2\text{ iff }|E_1|=|E_2| \and \forall {x<0 \Rightarrow x=y | x \in E_1, y \in E_2}
\end{equation}
