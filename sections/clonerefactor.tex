...
\ana{MOVE THIS, Part of the clone class collection and one that is found but not yet part of the collection. If the new one is a subset of the one in the collection then it is ignored. If the new one is a superset of any clone in the collection it replaces those. If one is a subset of the other it should be replaced.}
...
Although the duplicate relations allow for variability in the equality arrays as long as it falls under the threshold, negative numbers in the equality array may never vary. This is because negative numbers are the tokens/expressions that we do not allow variability in because they cannot be refactored in the same way.
\begin{equation}\label{eq:t2rcloneequality}
E_1\text{ can only be cloned with }E_2\text{ iff }|E_1|=|E_2| \land \forall (x \in E_1, y \in E_2) x<0 \lor y<0 \Rightarrow x=y
\end{equation}
...
Where \textit{C} denotes the clone class and \textit{I} denotes the clone instance. As the graph is traversed starting at the end, equal to graph traversal during clone detection, the last node in

\subsubsection{Try to Expand}
The problem with the graph clone detection technique is that
