...
The most used technique to refactor clones is ``Extract Method'' (creating a new method on basis of the contents of clones). However, method extraction cannot be applied in all cases. In some instances, more conditions may apply to be able to conduct a refactoring, if beneficial at all.
...
Clones that are not in the ``Can be extracted'' are not per definition not refactorable using method extraction. However, they may require additional transformations, other refactoring techniques, etc. We further explain opportunities to refactor such clones outside the ``Can be extracted'' category in section \ref{sec:discussrefactorability}.
