...
\todo{These equality arrays do not include the ( and ) separators. This is not completely right. Should we change it?}
\begin{figure}[H]
\begin{parcolumns}{3}
\colchunk[1]{
\begin{javacode}
...
Where \textit{C} denotes the clone class and \textit{I} denotes the clone instance. The graph is traversed, starting from the last node in the code fragment (equal to graph traversal during clone detection). Because of that, $C_1$ $I_1$ is the very last clone instance in this example. 
...
\end{javacode}}
\end{parcolumns}
\caption{Type 2R clones with equality arrays and variability percentages.}
\label{fig:trytoexpand}
\end{figure}

As an example, consider the clone fragment in figure \ref{fig:trytoexpand}.
