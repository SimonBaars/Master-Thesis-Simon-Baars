...
Figure \ref{fig:tokenvolume} shows the obtained results when plotting the clone volume vs the maintainability increase/decrease. We define the clone volume as the combined number of tokens in all clone instances in a refactored clone class. On the secondary axis (blue line) we plot the amount of times a clone class with the specified amount of tokens is refactored (e.g. the amount of data points the data is based on). As the amount of data points decreases, the datapoints gain less statistical significance.
...
The tokens volume of the clone is equal to the decrease of duplicated tokens. Because of that, what we see in Figure~\ref{fig:tokenvolume} could largely be caused by this metric. Because of that, we also plot graphs in which each metric is considered singularly. In Figure~\ref{fig:}
