...
\subsubsection{Detecting Clones}
...
Using the example shown in figure \ref{fig:clonegrid} we can explain how we detect clones. Suppose we are currently finding clones at the purple marked box. We then follow all ``duplicate'' relations until we have found all clones (the boxes marked in green). We now have a single statement clone class. Next, we move to the previous line. Here again, we collect all duplicates of this node. For each of these duplicates, we check whether the node following it is already in the clone class we collected in the previous iteration. In this case, only two of the 
