...
As a first step towards building the clone graph, we preprocess the AST to decide which AST nodes should become part of the clone graph. We filter the following types of nodes:
\begin{itemize}
  \item \textbf{Expressions: }The smallest entity in the clone graph is a Statement. Because of that, we filter out all expressions.
  \item \textbf{Package declaration and import statements: }These are omitted by most clone detection tools, as clones in import statements hold limited valuable information.
  \item \textbf{Modifiers: }The clone graphs' nodes consist of only declarations and statements. Modifiers are part of the declarations, so they are not considered separately.
\end{itemize}

\paragraph{Building the clone graph}
...
In the definition of type 2R (see section \ref{eq:type2r}) clones we described how type 2R clones work on basis of a variability threshold. This threshold, shown more formally in formula \ref{eq:type2r}, is checked by CloneRefactor. Implementing such a threshold involves some important design decisions, and has a lot of underwater complexity. In this section we explain the way CloneRefactor uses to verify the type 2R variability threshold.
