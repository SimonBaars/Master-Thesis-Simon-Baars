...
\section{Relation}
Table \ref{tab:full-relation} shows average scores and metrics for all refactored clones with the specified relation. The \textbf{Relation} column refers to the relations introduced in Section~\ref{sec:setuprelation}. Relations are mutually exclusive (as described in Section~\ref{sec:setuprelation}), so all percentages add up to 100\%.
