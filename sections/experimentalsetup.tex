...
For our experiments we use a large corpus of open source projects \cite{githubCorpus2013}. %Sander: Miss wil je hier zeggen dat het gemaakt is door Allamanis et al ofziets
This corpus contains a set of Java projects from GitHub, selected by number of forks. The projects in this corpus were de-duplicated manually. % Sander: Wat werd er ge-de-dupliceerd? de files? de projects?
% Sander: Kan ik de corpus ergens vinden?
...
% Sander:huh wat je zegt nu eerst dat hij useful is, en daarmee meteen dat we hem hebben veranderd. waarom hebben we die controlled set nodig?
...
Because we cloned the projects (and do not use the sources provided in the corpus) % Sander:Het is hier niet duidelijk wat je nog met corpus bedoelt. Jouw corpus? die van allamanis?
, they still included generated source code. Because we do not want to analyze generated Java files, we decided to further filter the corpus to only include projects with a conventional structure. For Maven, projects are recommended to put their production code in the folder structure \texttt{src/main/java}. We omitted Maven projects that do not adhere to this convention. % Sander: weten we nu zeker dat er geen generated files meer in staan?

\subsection{Gathering Dependencies}
As a next step, we collect all dependencies of the downloaded software projects. Maven uses the following command to automate this process: \texttt{mvn dependency:copy-dependencies -DoutputDirectory=lib}. This process can % Sander: can fail of will fail?
...
To verify the correctness of all Java files in the projects, we used JavaParser \cite{tomassetti2017javaparser} to create an AST of every Java file in the \texttt{src/main/java} folder of the software projects. A very small set of projects had syntactical errors, which we removed from the corpus. % Sander:examples of syntactical error? why did we remove those -> they crash the javaparser

\subsection{Inspecting outliers}
We ran CloneRefactor over every project in the corpus and inspected the output for each project. Some projects gave unusual high numbers of projects % Sander: clones?
...
\begin{itemize}
  ...
  \item Normalizing each obtained metric delta over all deltas obtained a metric in our dataset results in equally weighted scores. % Sander: zin klopt niet (en snap hem ook niet)
\end{itemize}
We derived these assumptions partly from Heitlager et al~\cite{heitlager2007practical} and our own expert intuition. The validity of these metrics influences the validity of the conclusions we can draw from our data. Because of that, in our experiments we focus mainly on significant differences in maintainability to draw a conclusion. % Sander: miss wil je deze laatste twee zin weghalen om jezelf niet in je voet te schieten, als je het maar kan beantwoorden tijdens je defense
...
\end{equation}
% Sander: miss mooier met Maintainability = ....
