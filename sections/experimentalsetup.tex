...
A clone class is flagged as ``Same method'' if all clone instances in the clone class are in the same method. We check this relation for each node in a clone instance with each other node in each other clone instance within a clone class. We define the method as the first method declaration that is found when tracing  from a node in the clone instance. Please note that a clone instance may not always be in a method, for which this predicate will fail. Each node in a clone instance is checked for this predicate, so if a single node of a clone instance is outside a method, the predicate will fail.

A clone class is flagged as ``Same class'' if all clone instances in the clone class are in the same class. We define the class as the first class declaration that is encountered when following the parent nodes in the ast for a given cloned node.
...
\begin{equation}\label{eq:sameclass}
\forall (i_1 \in C)\text{ }\forall (i_2 \in C)\text{ }Si_1 = Di_2 \lor Si_2 = Di_1
\end{equation}
