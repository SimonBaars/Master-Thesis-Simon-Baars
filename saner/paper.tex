...
\item \textbf{Correctly identifies .} In order to detect such clones that can be refactored, we need to be able to extend the corresponding tool accordingly.
...
Apart from these criteria, we found that the output of these clone detection tools cannot be post-processed to find the clone types proposed in Sec.~\ref{sec:rtypes}. This is mainly because the clones of T2R are not a subset of type 2 clones and will thus require an analysis of the entire system. % TODO
