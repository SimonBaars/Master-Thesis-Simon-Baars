...
In this section, we describe relevant code clone and clone refactoring research.
...
To investigate the impact of the definitions introduced in Sec.~\ref{sec:research}, we need a tool to detect refactorable clones according to these definitions. We attempt to reuse an existing tool, thus we search for candidates. None are found, thus we build a new tool. 

\subsection{Survey existing clone detection tools}
We conducted a short survey on (recent) clone detection tools to detect such clones that can be refactored. The results of our survey are displayed in table~\ref{table:dettools}. We chose a set of tools that are open source and can analyze a popular object-oriented programming language. Next, we formulate the following four criteria by which we analyze these tools:
\begin{enumerate}
...
\item \textbf{Can be extended to the definitions of refactorable clones.} We assembled a number of control projects to assess the validity of clone detection tools.
...
Apart from these criteria, we found that the output of these clone detection tools cannot be post-processed to find the clone types proposed in Sec. \ref{sec:rtypes}. This is mainly because the clones of T2R are not a subset of type 2 clones and will thus require an analysis of the entire system.
...
%None of the surveyed tools~\cite{ragkhitwetsagul2019siamese, roy2008nicad, semura2017ccfindersw, sajnani2016sourcerercc, saini2018oreo, jiang2007deckard, kamalpriya2017enhancing, mazinanian2016jdeodorant} were suitable for our definitions of refactorable clones, because of which we propose CR\footnote{Name anonymized for the blind review, camera-ready paper would include a link to the GitHub repo of the tool.}. This tool goes through a 3-step process to automatically refactor clones as shown in Fig.~\ref{fig:clonerefactorflow}. In the following sections, we explain these steps.
