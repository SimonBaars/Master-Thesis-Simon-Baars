...
CloneRefactor uses JavaParser \cite{tomassetti2017javaparser}: an AST-parsing library that allows to modify the AST and write it back to source code. To refactor clones, CloneRefactor creates a new method declaration and moves all statements from the 

\subsection{Characteristics of the extracted method}
We define the following characteristics that have an impact on the refactoring of the clone class:
\begin{itemize}
\item \textbf{Statements}: The number of statements that \textit{each clone instance of the clone class} spans.
\item \textbf{Tokens}: The number of tokens that the \textit{each clone instance of the clone class} spans.
\item \textbf{Relation}: The relation category (Sec.~\ref{sec:relation}) that this clone class belongs to.
...
\subsection{Impact on maintainability metrics}
