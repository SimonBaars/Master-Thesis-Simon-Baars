...
Like the clone types that are commonly used in literature \cite{roy2007survey}, each type R clone is a subset of the next type. This property is displayed in equation \ref{eq:typerelation}. Each statement that is a T1R will also be found for T2R; each T2R statement will also be found for T3R. Each rule that applies to T1R also applies to T2R; each T2R rule applies to T3R.

\begin{equation}\label{eq:typerelation}
T1R \subseteq T2R \subseteq T3R
\end{equation}
...
To solve the issues identifier in Sec.~\ref{sec:type2}, we introduce . We tackle the previously outlined shortcomings of type 2 clones to be able to detect such clones that can and should be refactored. All rules that apply to T1R clones also apply to T2R clones. Additionally, T2R clones allow variability in literals, variables and method calls. This variability however is constrained by a threshold as a tradeoff exists in the refactoring of such differences. Furthermore, T2R clones allow any variability in method names and class/enum/interface names. This is because, if the body of a method/class/enum/interface equals the body of another, but only their names differ, this can be refactored without a design tradeoff.

When refactoring two fragments that differ by a few literals, called methods or used variables, we are faced with a design tradeoff. For instance, if two literals differ and we want to refactor by method extraction, we need our new method to have an added argument to which each usage can pass the literal that was used at that place. Likewise, such things can be done for different variables that have been used and different called methods. Important here is, that the type of the used literals/variables are equal. Likewise, the signature of the called methods must be equal.

To limit the negative impact of this tradeoff on the design of the system, we formalized as threshold for the variability between fragments. The formula by which this threshold is calculated is displayed in equation \ref{eq:type2r}. We take the amount of literals, variables and method calls that differ from other clone instances in a clone class and divide it by the total number of tokens in the clone instance. Based on this threshold, we decide whether a clone should be considered for refactoring.
...
$$l = \text{Literals in clone instance}$$
$$v = \text{Variables in clone instance}$$
$$m = \text{Method calls in clone instance}$$

A method call does often not consist of a single token (unlike variables and literals). For instance, a method call \texttt{System.out.println()} consists of several segments: a type reference to the \texttt{System} type, a reference to the static \texttt{out} field and a call of the \texttt{println()} method. Because of this, T2R clones are not a subset of of type 2 clones. Because T3R clones inherit all rules of T2R clones, T3R clones are also not a subset of T3 clones. This is displayed in equation \ref{eq:typerelation2}.

\begin{equation}\label{eq:typerelation2}
T1R \subseteq T1 \text{, but } T2R	\nsubseteq T2 \text{ and } T3R \nsubseteq T3
\end{equation}

\subsubsection{T3R clones}\label{sec:type3r}
Type 3 clones allow any change in statements (added, removed and changed statements). Many clone detection tools have implemented this type by introducing a similarity threshold \cite{roy2008nicad,ragkhitwetsagul2019siamese,jiang2007deckard,semura2017ccfindersw}. This similarity threshold entails that two fragments may differ by a certain percentage. This means that T3R allows the inclusion of a statement that is not detected by type 1 or 2 clone detection. When looking at how we can refactor a statement that is not included by one clone instance but is in another, we find that we require a conditional block to make up for the difference in statements. This is a tradeoff, as an added conditional block increases the complexity of the system. Because of that, we defined T3R clones in such a way that they are directed towards finding clones that are worth this tradeoff.
...
Whether a node is considered a clone depends on the clone type that is being analyzed. On basis of this graph we detect clone classes. We compare clone classes against the thresholds, and remove the clone classes that do not pass the test.
...
However, we would argue that going with this ``magic number 6'' eliminates a lot of harmful clones that should be refactored. For instance
...
Table~\ref{table:relations} contains our results regarding the relations between clone instances. In this table, ``T1'' stands for the type 1 algorithm from literature and ``1R'' stands for our T1R definition as explained in section \ref{sec:type1r}.
...
The next step is to integrate our T2R and T3R definitions into our CloneRefactor tool. We then will look into appropriate thresholds for these types of clones. We can then re-run all scripts for T2R and 3R clones. On basis of these results, we can prioritize implementing automated refactorings.
