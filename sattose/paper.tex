...
In this chapter, we will propose solutions for shortcomings in type 1, type 2 and type 3 clones to increase the chance that we can merge the clone while improving the design. Type-4 clones are, because of the serious challenges involved in their detection and refactoring, out of the scope for this study.
...
The next step will be to integrate our redefinitions of type 1, 2 and 3 clones into CloneRefactor. This way, we can run all scripts that we used for our measurements for these novel clone definitions. We are curious to see how it compares with the results we had so far.
