...
\textbf{RQ1.} How do we identify clones that we \textbf{can} refactor?\\
\textbf{RQ2.} How do we identify clones that we \textbf{should} refactor?\\
\textbf{RQ3.} To what extent can we \textbf{automate} the process of refactoring?\\

For \textbf{RQ1} we look into current clone definitions and clone detection methods. We then assess their suitability for refactoring purposes. For \textbf{RQ2} we look into to what extent clones can be filtered based on their impact on the system design when refactored. We do this by both determining good thresholds for the clone detection and by assessing the design implications of applying refactorings to the code. \textbf{RQ3} is currently still future work, hence the work in progress status of this paper.
...
Like the clone types that are commonly used in literature \cite{roy2007survey}, each type R clone is a subset of the next type. This property is displayed in equation \ref{eq:typerelation}. Each statement found as cloned for T1R clones, will also be found as cloned for T2R and T3R. Each rule that applies to T1R clones also applies to T2R and T3R.
...
\begin{itemize}
  \item \textbf{Referenced Types:} For all types references in the code, we should compare their FQI with other code fragments to ensure functional equality.
...
We tackle the previously outlined shortcomings of type 2 clones to be able to detect such clones that can and should be refactored. All rules that apply to type 1R clones also apply to type 2R clones. Additionally, type 2R clones allow variability in literals, variables and method calls. This variability however is constrained by a threshold as a tradeoff exists in the refactoring of such differences. Furthermore, type 2R clones allow any variability in method names and class/enum/interface names. This is because, if the body of a method/class/enum/interface equals the body of another, but only their names differ, this can be refactored without a design tradeoff.
...
$$t = \text{Tokens in clone instance}$$
