...
\begin{itemize}
  ...
  \item \textbf{Treat method call expressions like literals:} As type-2 clones allows change in identifiers, also the names of called methods may vary. However, because of this, completely different methods can be called in cloned fragments as a result. Having many method calls in a cloned fragment, will decrease the need for refactoring (in terms of improving design) when these are matched as type-2 clones (so method identifiers differ), because they can describe a completely different functionality. Because of that, we recommend having variability in method call expressions under the same threshold as literals, so only a limited number of method calls can vary. 
\end{itemize}
