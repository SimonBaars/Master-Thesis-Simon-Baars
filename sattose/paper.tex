...
Regarding clone detection, there is a lot of variability in literature whether clone pairs or clone classes should be detected. In this study we focus on clone classes, because of the advantages for refactoring. Clone pairs do not provide a general overview of all entities containing the clones, with all their related issues and characteristics \cite{fontana2012duplicated}. Although clone classes are harder to manage, they provide all information needed to plan a suitable refactoring strategy, since this way all instances of a clone are considered. There is also another issue that results from grouping clones by pairs: clone reference amount increases according to the binomial coefficient formula (two clones form a pair, three clones form three pairs, four clones form six pairs, and so on), which causes a heavy information redundancy \cite{fontana2012duplicated}.
...
\begin{enumerate}
    \item \textbf{Should find clones in any context.} Some tools only find clones in specific contexts, such as only method-level clones. We want to perform an analysis on all clones in projects to get a complete overview. Because of that, we want to use a clone detection tool that is able to find clones in any context.
...
As indicated in section \ref{chap:challenge} CloneRefactor requires all libraries of software projects we test. As these are not included in the used corpus \cite{githubCorpus2013}, we decided to filter the corpus to only include Maven projects. Maven is a build automation tool used primarily for Java, and works on basis of an \texttt{pom.xml} file to describe the projects' dependencies. As no \texttt{pom.xml} files are included in the corpus, we cloned the latest version of each project in the corpus. We then removed each project that contained no ``pom.xml'' file (a file that describes the dependencies used by the project). We then filtered the corpus further to make sure each project contained a ``src/main/java'' folder, to be sure no test files or other redundant sources are included in the clone detection. As a final step, we collected all dependencies for each project by using the \texttt{mvn dependency:copy-dependencies -DoutputDirectory=lib} Maven command, and removed each project for which not all dependencies were available (due to non-Maven dependencies being used or unsatisfiable dependencies being referenced in the ``pom.xml'' file).
...
\begin{tabular}{|l|l|l|}
\hline
Extra & \textbf{Type 1} & \textbf{Type 1R} \\ \hline
...
Looking at table \ref{table:clonedet}, it becomes apparent that the type 1R algorithm finds significantly less clones found than the type 1 algorithm. Figure \ref{fig:type1rclone} displays an example of a clone that does get detected by the type 1 algorithm but does not get detected by the type 1R algorithm. This is because the variable ``key'' and ``prefix'' are of a different primitive type between the clone instances. This makes this clone less suitable for automated refactoring. We manually checked some differences between the detected type 1 clones and type 1R clones and in each instance there was either a variable with different type, a different method call or a different type (but textual equality). This indicates that about a third of the clones have textual equality, but are not actually equal when considering the types of expressions.

\begin{figure}[H]
  \caption{Example of a clone that is not a type 1R clone but is a type 1 clone.}
  \medskip
    \includegraphics[width=1\columnwidth]{img/clone}
  \label{fig:type1rclone}
\end{figure}
...
\textbf{Relation}  & \textbf{\# (1)} & \textbf{\% (1)}  & \textbf{\# (1R)} & \textbf{\% (1R)} \\ \hline
...
\begin{tabular}{|l|l|l|l|l|}
  ...
  Partial Method                                                           & 32,214 & 56.07 & 19,264 & 56.07 \\ \hline
  Several Methods                                                          & 10,542  & 26.41 & 19,264 & 56.07 \\ \hline
  Includes Constructor                                                     & 1,772  & 4.45  & 19,264 & 56.07 \\ \hline
  Includes Field                                                           & 1,681  & 3.34  & 19,264 & 56.07 \\ \hline
  Partial Constructor                                                      & 1,389  & 3.20  & 19,264 & 56.07 \\ \hline
  Only Fields                                                              & 962    & 1.64  & 19,264 & 56.07 \\ \hline
  Full Method                                                              & 647    & 1.61  & 19,264 & 56.07 \\ \hline
  \begin{tabular}[c]{@{}l@{}}Includes Class\\ Declaration\end{tabular}     & 445    & 1.30  & 19,264 & 56.07 \\ \hline
  Other                                                                    & 369    & 1.07  & 19,264 & 56.07 \\ \hline
  Full Class                                                               & 192    & 0.56  & 19,264 & 56.07 \\ \hline
  \begin{tabular}[c]{@{}l@{}}Includes Enum\\ Constant\end{tabular}         & 52     & 0.15  & 19,264 & 56.07 \\ \hline
  \begin{tabular}[c]{@{}l@{}}Includes Enum\\ Declaration\end{tabular}      & 47     & 0.14  & 19,264 & 56.07 \\ \hline
  \begin{tabular}[c]{@{}l@{}}Includes Interface\\ Declaration\end{tabular} & 10     & 0.03  & 19,264 & 56.07 \\ \hline
  Full Interface                                                           & 6      & 0.02  & 19,264 & 56.07 \\ \hline
  Full Enumeration                                                         & 4      & 0.01  & 19,264 & 56.07 \\ \hline
\end{tabular}
\end{center}
\end{table}
...
The most trivial way to merge a clone is through method extraction. However, method extraction is not always possible. For instance, if a part of a subtree (in a programs' AST) is matched as a clone or if the control flow (return, break and continue statements) prohibit extraction. Because of this, we chose to measure what percentage of ``Partial Method'' clones are refactorable using method extraction. Our results are displayed in table \ref{table:refactorability}. In this table we use the following categories:
\begin{itemize}
    ...
    \item \textbf{Complex control flow:} This clone contains break, continue or return statements.
    \item \textbf{Spans part of a block:} This clone spans a part of a block, including the declaration. For instance, when an if-statement is matched but not the entirety of its body.
    \item \textbf{Is not a partial method:} If the clone does not fall in the ``Partial method'' category of table \label{table:contents}, the ``extract method'' refactoring technique cannot be applied.
\end{itemize}
