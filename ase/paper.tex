...
We use software maintainability metrics to determine the impact of a refactoring applied to duplicate code. For a given refactoring we aggregate a set maintainability metrics (duplication, complexity, unit size, code volume) that are calculated on unit level, which we call the delta maintainability score. Based on this score, we extract common factors between clones for a developer to filter their clone output based on the factors and the derived thresholds for them.

% Experimental setup: how we validated that we fixed the problem
To conduct such refactorings in a controlled environment, we create a tool that can automatically refactor a set of duplication problems in Java source code. This set is selected because not all clones can easily be automatically refactored, so we define those that can. Using these definitions we can extract a sufficiently large dataset of refactored clones to perform our analyses. Our tool refactors 13k clones over 2k open-source Java projects and measures the delta maintainability scores of the refactoring.
...
\subsection{Difference from literature clone types}
These proposed definitions slightly differ from type 1, 2 and 3 clones as defined in literature \cite{roy2007survey}. To compare them, we refer to the literature types as T1, T2 and T3 and towards our as T1R (contextually equal clones), T2R (allowing variability) and T3R (gapped clones). Like the literature types, these clone type definitions are subsets of each other:
...
Where $N_{\text{clone type}}$ is the set of cloned lines found by the rules of the specified clone type. All clones found for type 1R will also be found for type 2R and type 3R.
...
None of the surveyed tools had all the desired characteristics. Also, the output of these clone detection tools cannot be post-processed to match the clone definition proposed in Sec.~\ref{sec:research}. This is mainly because the automatically refactorable clones are not necessarily a subset of clones detected by these tools and will thus require an analysis of the entire system.
...
% \subsection{Survey of existing clone detection tools}\label{sec:survey}
